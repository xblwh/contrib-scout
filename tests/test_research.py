...
def issue(**changes):
    return {"number": 42, "title": "Python streaming parser loses final chunk", "body": "Python example: the final chunk is missing.",
            "html_url": "https://github.com/demo/project/issues/42", "state": "open", "labels": [{"name": "good first issue"}],
            "assignees": [], "created_at": "2026-09-01T00:00:00Z", "updated_at": "2026-09-13T00:00:00Z", **changes}


def pull(**changes):
    return {"number": 99, "title": "Preserve Python streaming parser final chunk", "body": "Fixes #42", "state": "open",
            "html_url": "https://github.com/demo/project/pull/99", **changes}
...
class FakeGitHub:
    ...
    def get(self, path):
        return {"full_name": "demo/project", "html_url": "https://github.com/demo/project", "private": False,
...
class ResearchTests(unittest.TestCase):
    def assess(self, item=None, pulls=None, timeline=None, comments=None, **kw):
        return assess(item or issue(), pulls or [], timeline or [], comments or [], "demo/project", ["Python"], now=NOW, **kw)

    def test_repo_normalization(self):
        self.assertEqual(parse_repo("https://github.com/pydantic/pydantic-ai.git/"), "pydantic/pydantic-ai")

    def test_only_repo_urls_accepted(self):
        for value in ["https://evil.test/o/r", "https://github.com/a/b/issues/1", "a/..", "a/b?x=1", "--help", "https://user@github.com/a/b", "http://127.0.0.1/a/b"]:
            with self.subTest(value=value), self.assertRaises(ResearchError):
                parse_repo(value)

    def test_open_linked_pull_holds_candidate(self):
        source = {"number": 99, "title": "Alternative implementation", "state": "open", "html_url": "https://github.com/other/project/pull/99", "pull_request": {}}
        result = self.assess(timeline=[{"event": "cross-referenced", "source": {"issue": {**source, "pull_request": {"url": "api"}}}}])
    ...
    def test_foreign_repo_shorthand_is_not_same_issue(self):
        result = self.assess(pulls=[pull(title="Entirely unrelated", body="Fixes other/project#42")])
        self.assertEqual(result["status"], "investigate")

    def test_issue_number_boundary(self):
        result = self.assess(pulls=[pull(title="Entirely unrelated", body="Fixes #420")])
    ...
    def test_assigned_issue_on_hold(self):
        self.assertEqual(self.assess(issue(assignees=[{"login": "alice"}]))["status"], "hold")

    def test_claim_comment_creates_cited_risk(self):
        result = self.assess(comments=[{"id": 3, "body": "I'd like to work on this issue.", "html_url": "https://github.com/demo/project/issues/42#issuecomment-3"}])
        self.assertEqual(result["status"], "review")
        self.assertEqual(result["sources"][1]["id"], "claim-3")

    def test_withdrawn_claim_not_treated_as_current(self):
        result = self.assess(comments=[{"id": 3, "body": "I'm no longer working on this. Please unassign me.", "html_url": "https://github.com/demo/project/issues/42#issuecomment-3"}])
        self.assertEqual(result["status"], "investigate")
    ...
    def test_merged_related_pull_requires_check(self):
        result = self.assess(timeline=[{"event": "cross-referenced", "source": {"issue": {
            "number": 99, "title": "Fix this", "state": "closed", "html_url": "https://github.com/demo/project/pull/99",
    ...
    def test_stale_issue_not_presented_as_fresh(self):
        self.assertEqual(self.assess(issue(updated_at="2024-01-01T00:00:00Z"))["status"], "review")
    ...
    def test_issues_endpoint_filters_pull_requests_and_discloses_window(self):
        ...
        self.assertTrue(any("CONTRIBUTING" in message for message in report["warnings"]))
