...
def analysis():
    return {"analyses": [{"issue_number": 42, "summary": "需要复现末尾 chunk 丢失。", "suggested_scope": "先确认流式读取边界。",
                          "verification_plan": "添加覆盖结束信号的回归案例；尚未执行。", "source_ids": ["issue-42"]}]}
...
class LLMTests(unittest.TestCase):
    ...
    def test_other_candidates_evidence_not_allowed(self):
        self.report["candidates"].append({**copy.deepcopy(self.report["candidates"][0]), "number": 43,
                                          "sources": [{"id": "issue-43"}]})
        value = analysis()
    ...
    def test_missing_duplicate_and_unknown_issues_rejected(self):
        for value in [{"analyses": []}, {"analyses": analysis()["analyses"] * 2},
    ...
    def test_malformed_fields_rejected(self):
        for field, invalid in [("summary", []), ("source_ids", [None]), ("verification_plan", ""), ("issue_number", True)]:
            value = analysis()
            value["analyses"][0][field] = invalid
            with self.subTest(field=field), self.assertRaises(ResearchError):
                validate_analysis(value, self.report)

    @patch.dict(os.environ, {"SCOUT_LLM_BASE_URL": "https://model.example/v1", "SCOUT_LLM_API_KEY": "test-key", "SCOUT_LLM_MODEL": "test-model"})
    def test_mocked_model_request_preserves_facts_and_records_usage(self):
        before = copy.deepcopy(self.report["candidates"][0])
        response = {"choices": [{"message": {"content": json.dumps(analysis())}}], "usage": {"prompt_tokens": 100, "completion_tokens": 50, "total_tokens": 150}}
        with patch("contrib_scout.llm.urlopen", return_value=io.BytesIO(json.dumps(response).encode())) as mocked:
            enhance(self.report)
    ...
    @patch.dict(os.environ, {"SCOUT_INPUT_PRICE_PER_MILLION": "2", "SCOUT_OUTPUT_PRICE_PER_MILLION": "8"})
    def test_cost_uses_user_prices(self):
        self.assertEqual(estimated_cost({"prompt_tokens": 1_000_000, "completion_tokens": 500_000}), 6)
