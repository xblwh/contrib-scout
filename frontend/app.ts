...
type Job = Partial<Result> & { state: string; error?: string };
...
function render(result: Result): void {
  ...
  report.documents.forEach((doc) =>
    documents.append(link(doc.path + " ↗", doc.url)),
  );
...
$("research-form").addEventListener("submit", async (event) => {
  ...
  busyUI(true);
  feedback(
    "正在读取仓库文档、issue、评论和相关 PR。大仓库可能需要几分钟，请保持页面打开。",
    "loading",
  );
  ...
  try {
    ...
    while (Date.now() - started < 900_000) {
      ...
      const result = await api<Job>(`/api/jobs/${job.job_id}`);
      ...
      feedback(
        `正在核对来源… 已用 ${Math.floor((Date.now() - started) / 1000)} 秒。`,
        "loading",
      );
    }
    if (!completed)
