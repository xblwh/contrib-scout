...
4. 安装开发工具：`python3 -m pip install -e ".[dev]"`。运行 `ruff check contrib_scout tests`、`ruff format --check contrib_scout tests`、`python3 -m unittest discover -s tests -v`、`npm run check` 和 `git diff --check`。
