...
class GitHub:
    ...
    def get(self, path, optional=False):
        ...
        if self.use_gh:
            try:
                ...
                raise ResearchError("无法运行 gh；请检查安装和登录，或去掉 --use-gh。") from exc
            if result.returncode:
                if optional and "(HTTP 404)" in result.stderr:
                    return None
                raise ResearchError("GitHub 请求失败；请检查仓库、gh 登录状态和 API 额度。")
            try:
                return json.loads(result.stdout)
            except ValueError as exc:
                raise ResearchError("GitHub 返回了无效 JSON。") from exc
        headers = {"Accept": "application/vnd.github+json", "User-Agent": "contrib-scout/0.1",
        ...
        try:
            ...
            if exc.code == 404 and optional:
                return None
            messages = {401: "GitHub Token 无效。", 403: "GitHub 拒绝请求或额度不足；请配置 Token 后重试。",
                        404: "仓库不存在或不可访问。", 429: "GitHub 请求过于频繁，请稍后重试。"}
            raise ResearchError(messages.get(exc.code, f"GitHub 请求失败（HTTP {exc.code}）。")) from exc
    ...
    def document(self, repo, path):
        ...
        if not data or not isinstance(data, dict) or data.get("encoding") != "base64":
            return None
        try:
            content = base64.b64decode(data.get("content", ""), validate=False).decode("utf-8", errors="replace")
        except (ValueError, TypeError) as exc:
            raise ResearchError("贡献文档内容无法解码。") from exc
        return {"path": data.get("path", path), "url": data.get("html_url", ""),
                "text": content[:24000], "truncated": len(content) > 24000}
