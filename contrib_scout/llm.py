...
def configured():
    return bool(os.getenv("SCOUT_LLM_BASE_URL") and os.getenv("SCOUT_LLM_MODEL") and os.getenv("SCOUT_LLM_API_KEY"))
...
def validate_analysis(data, report):
    ...
    for item in data["analyses"]:
        ...
        if isinstance(number, bool) or not isinstance(number, int) or number not in candidates or number in result:
            raise ResearchError("AI 输出了未知或重复的 issue，已丢弃本次分析。")
        allowed = allowed_docs | {source["id"] for source in candidates[number]["sources"]}
        ...
        result[number] = {key: item[key] for key in ("summary", "suggested_scope", "verification_plan", "source_ids")}
...
def estimated_cost(usage):
    values = (os.getenv("SCOUT_INPUT_PRICE_PER_MILLION"), os.getenv("SCOUT_OUTPUT_PRICE_PER_MILLION"))
    if not all(values):
        return None
    try:
        ...
        if any(not math.isfinite(value) or value < 0 for value in prices):
            return None
        return round((usage.get("prompt_tokens", 0) * prices[0] + usage.get("completion_tokens", 0) * prices[1]) / 1_000_000, 6)
...
def enhance(report):
    if not configured():
        raise ResearchError("AI 尚未配置：请在启动服务的终端设置 SCOUT_LLM_BASE_URL、SCOUT_LLM_MODEL、SCOUT_LLM_API_KEY。")
    ...
    source_data = {"stack": report["stack"], "repository": report["repository"],
                   "documents": [{**doc, "text": doc["text"][:6000]} for doc in report["documents"]],
                   "candidates": report["candidates"]}
    payload = {"model": model, "max_completion_tokens": 4000, "response_format": {"type": "json_object"},
    ...
    request = Request(base_url + "/chat/completions", method="POST", data=json.dumps(payload).encode(),
                      headers={"Authorization": "Bearer " + os.environ["SCOUT_LLM_API_KEY"], "Content-Type": "application/json"})
    ...
    try:
        ...
        raise ResearchError(f"模型服务返回 HTTP {exc.code}；请检查密钥、模型名称和 JSON 输出支持。") from exc
    except (URLError, TimeoutError, ValueError, KeyError, IndexError, TypeError) as exc:
        raise ResearchError("模型请求失败或 JSON 输出无效；GitHub 事实报告仍保留。") from exc
    for candidate in report["candidates"]:
        candidate["ai"] = analyses[candidate["number"]]
    usage = raw.get("usage") or {}
    report["ai"] = {"enabled": True, "model": model, "elapsed_seconds": round(time.monotonic() - started, 2),
