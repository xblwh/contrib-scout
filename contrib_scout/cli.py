...
def main(argv=None):
    ...
    scan.add_argument("repo", nargs="?", help="owner/repo 或 GitHub 仓库 URL")
    ...
    scan.add_argument("--use-gh", action="store_true", help="使用本机已登录的 GitHub CLI")
    scan.add_argument("--ai", action="store_true", help="将公开调研材料发送给配置的模型服务")
    scan.add_argument("--demo", action="store_true", help="使用明确标注的虚构演示数据，不发网络请求")
    ...
    try:
        ...
        report = demo_report() if args.demo else research(args.repo, args.stack, args.limit, GitHub(args.use_gh))
        if args.ai:
            try:
                enhance(report)
